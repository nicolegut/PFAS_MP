...
"""
Okay so the additional rows are due to a many to one merge, 
maybe more points is not a bad thing? 
if they serve mulitple zips it is additonal data
however, some of the zips don't exist on our map 
might want to bring in hazen data and validate?
check if there are only duplicate zips for the same PWSID
^that would be okay
if there are different PWSID for the same zip - issue with which data to keep 
"""
