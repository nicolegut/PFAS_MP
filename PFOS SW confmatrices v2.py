...
# parameter tables
PFOS_SW_IDW = os.path.join(PFOS_SW_folder, "IDW_5_Params_ssw.csv")
PFOS_SW_UK = os.path.join(PFOS_SW_folder, "UK_5_Params_ssw.csv")
PFOS_SW_OK = os.path.join(PFOS_SW_folder, "OK_5_Params_ssw.csv")
...
for idx, row in IDW_params.iterrows():
    ...
    recall_contam = (cm[1, 1]) / (cm[1].sum())

    print(f"\n IWD Raster: {out_name}")
    print("Observed (rows) vs Predicted (columns)")
    print("Classes: 0 =< 0.004, >0.004")
    print(cm)
